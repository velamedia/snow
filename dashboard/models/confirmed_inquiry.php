...
class Quotation_comparison extends MY_Model {
	...
	public function confirm_quotation($quotation_id,$accepted_quotes,$item_ids,$supplier_ids)
	{
		//get the inquiry id
		...
		foreach ($item_ids as $item_id) {
			$accepted_price = ($accepted_quotes[$item_id])? $accepted_quotes[$item_id]: 0;
			$supplier_id = ($accepted_quotes[$item_id])? $supplier_ids[$i]: 0;

			$data = array('accepted_price' => $accepted_price, 'supplier_id' => $supplier_id);
